def average_values_across_classes(
    unique_values_bonds, unique_values_angles, outputfilefolder
):
    """
    ...
    for i in range(0, (len(unique_values_bonds))):
        for j in range((i + 1), len(unique_values_bonds)):
            ...
            if (
                    unique_values_bonds[i][0] == unique_values_bonds[j][0] and
                    unique_values_bonds[i][1] == unique_values_bonds[j][1] or
                    ((unique_values_bonds[i][0] == unique_values_bonds[j][1]) and (unique_values_bonds[i][1] == unique_values_bonds[j][0]))
            ):
    ...
    for i in range(0, len(unique_values_bonds)):
        if (i in ignore_rows) == 0:
            ...
            fid.write(
                unique_values_bonds[i][0] +
                '-' +
                unique_values_bonds[i][1] +
                '  ' +
                str("%.2f" % unique_values_bonds[i][2]) +
                '  ' +
                str("%.3f" % unique_values_bonds[i][3]) +
                '\n'
            )
    fid.close()
    ...
    for i in range(0, (len(unique_values_angles))):
        for j in range((i + 1), len(unique_values_angles)):
            ...
            if (
                    (
                        unique_values_angles[i][0] == unique_values_angles[j][0] and
                        unique_values_angles[i][1] == unique_values_angles[j][1] and
                        unique_values_angles[i][2] == unique_values_angles[j][2]
                    ) or
                    (
                        unique_values_angles[i][0] == unique_values_angles[j][2] and
                        unique_values_angles[i][1] == unique_values_angles[j][1] and
                        unique_values_angles[i][2] == unique_values_angles[j][0]
                    )
            ):
    ...
    for i in range(0, len(unique_values_angles)):
        if (i in ignore_rows_angles) == 0:
            ...
            fid.write(
                unique_values_angles[i][0] +
                '-' +
                unique_values_angles[i][1] +
                '-' +
                unique_values_angles[i][2] +
                '  ' +
                str("%.2f" % unique_values_angles[i][3]) +
                '  ' +
                str("%.3f" % unique_values_angles[i][4]) +
                '\n'
            )
    fid.close()
