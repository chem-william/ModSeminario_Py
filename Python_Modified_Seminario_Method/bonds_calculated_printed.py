...
def bonds_calculated_printed(
    ...
    with open((outputfilefolder + 'Modified_Seminario_Bonds'), 'w') as fid:
        ...
        for i in range(0, len(bond_list)):
            AB = force_constant_bond(
                bond_list[i][0], bond_list[i][1], eigenvalues, eigenvectors, coords
            )
            BA = force_constant_bond(
                bond_list[i][1], bond_list[i][0], eigenvalues, eigenvectors, coords
            )
